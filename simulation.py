...
def run_all_simulations(results_df, n_simulations=10000):
    ...
    for _, row in results_df.iterrows():
        ...
        sim    = run_monte_carlo(P, n_simulations)

        simulation_results.append({
            "item":          row["item"],
            "discount_hour": row["discount_hour"],
            "day_type":      row.get("day_type", "All"),   # ← carry through
            "sold_rate":     sim["sold_rate"],
            "waste_rate":    sim["waste_rate"]
        })
